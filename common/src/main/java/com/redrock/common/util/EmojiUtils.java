package com.redrock.common.util;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

/**
 * Created by Jay on 2017/9/12.
 */

public class EmojiUtils {
    public static final String ENCODE_REGEX = "(?:[\uD83C\uDF00-\uD83D\uDDFF]|[\uD83E\uDD00-\uD83E\uDDFF]" +
            "|[\uD83D\uDE00-\uD83D\uDE4F]|[\uD83D\uDE80-\uD83D\uDEFF]|[\u2600-\u26FF]\uFE0F?" +
            "|[\u2700-\u27BF]\uFE0F?|\u24C2\uFE0F?|[\uD83C\uDDE6-\uD83C\uDDFF]{1,2}" +
            "|[\uD83C\uDD70\uD83C\uDD71\uD83C\uDD7E\uD83C\uDD7F\uD83C\uDD8E\uD83C\uDD91-\uD83C\uDD9A]\uFE0F?" +
            "|[\u0023\u002A\u0030-\u0039]\uFE0F?\u20E3|[\u2194-\u2199\u21A9-\u21AA]\uFE0F?" +
            "|[\u2B05-\u2B07\u2B1B\u2B1C\u2B50\u2B55]\uFE0F?|[\u2934\u2935]\uFE0F?" +
            "|[\u3030\u303D]\uFE0F?|[\u3297\u3299]\uFE0F?" +
            "|[\uD83C\uDE01\uD83C\uDE02\uD83C\uDE1A\uD83C\uDE2F\uD83C\uDE32-\uD83C\uDE3A\uD83C\uDE50\uD83C\uDE51]\uFE0F?" +
            "|[\u203C\u2049]\uFE0F?|[\u25AA\u25AB\u25B6\u25C0\u25FB-\u25FE]\uFE0F?" +
            "|[\u00A9\u00AE]\uFE0F?|[\u2122\u2139]\uFE0F?|\uD83C\uDC04\uFE0F?|\uD83C\uDCCF\uFE0F?" +
            "|[\u231A\u231B\u2328\u23CF\u23E9-\u23F3\u23F8-\u23FA]\uFE0F?)";

    public static final String DECODE_REGEX = "!\\[emoji&[a-f|0-9]{4}\\$[a-f|0-9]{4}]";

    private EmojiUtils() {}

    public static String encodeEmoji(String str) {
        Pattern pattern = Pattern.compile(ENCODE_REGEX);
        Matcher matcher = pattern.matcher(str);
        StringBuilder builder = new StringBuilder(str);

        while (matcher.find()) {
            String temp = matcher.group();
            String replace = "![emoji&" + Integer.toHexString(temp.charAt(0)).toLowerCase()
                    + "$" + Integer.toHexString(temp.charAt(1)) + "]";
            int start = builder.indexOf(temp);
            int end = start + temp.length();
            builder.replace(start, end, replace);
        }

        return builder.toString();
    }

    public static String decodeEmoji(String str) {
        //适配web的坑爹编码
        str = str.replaceAll("&amp;", "&");

        Pattern pattern = Pattern.compile(DECODE_REGEX);
        Matcher matcher = pattern.matcher(str);
        StringBuilder builder = new StringBuilder(str);

        while (matcher.find()) {
            String temp = matcher.group();
            String[] codes = temp.replaceAll("!\\[emoji&|]", "").split("\\$");
            int start = builder.indexOf(temp);
            int end = start + temp.length();
            String replace = "" + codeToChar(codes[0]) + codeToChar(codes[1]);
            builder.replace(start, end, replace);
        }

        return builder.toString();
    }

    private static char codeToChar(String code) {
        return Character.toChars(Integer.parseInt(code, 16))[0];
    }
}
